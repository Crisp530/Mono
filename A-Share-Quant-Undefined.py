...
if not all([GEMINI_API_KEY, TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID]):
    raise ValueError("⚠️ 环境变量未完全配置，请检查 GitHub Secrets！")
...
# 任务目标
请基于以下输入的股票信息，进行深度的多步逻辑推理，评估股票 {stock_name}({stock_code}) 是否具有建仓价值。
...
结合 {news_text}，评估该公司的核心护城河是否真实存在，并分析其是否深度契合 {report_theme} 主线逻辑。对于“出海”或“科技”逻辑，必须重点评估其面临的地缘政治风险及供应链脆弱性。
*请给出深度定性分析。*
...
def weekly_rebalance_job():
    ...
    for index, row in candidate_stocks.iterrows():
        ...
        ai_report = ai_qualitative_analysis(stock_name, stock_code, pe, roe, pct_chg, turnover, news_text)
